document.querySelectorAll(".form-select-chosen").forEach(element => {
  try {
    ...
    const outputWrap = element.querySelector(
    ...
    !outputWrap &&
      console.error("not .form-select-chosen__output-wrap: ", element);
    !blockSearch && console.error("not .form-select-chosen__search: ", element);
    !inputSearch &&
      console.error("not .form-select-chosen__search-input: ", element);
    !blockOptions &&
      console.error("not .form-select-chosen__options: ", element);
    ...
    document.addEventListener("click", e => {
      if (
        isVisible &&
        !isSync &&
        e.target.closest(".form-select-chosen__search") !== blockSearch
      ) {
        isVisible = false;
    ...
    function searchValues(str) {
      let count = 0;
      blockOptions
        .querySelectorAll(".form-select-chosen__option:not(.is-not-search)")
        .forEach(element => {
      ...
      count
        ? blockOptions.classList.remove("is-empty")
        : blockOptions.classList.add("is-empty");
    }

    function addEvent(element) {
      element.classList.contains("form-select-chosen__option") &&
        element.addEventListener("click", function (event) {
          ...
          blockOptions
            .querySelectorAll(".form-select-chosen__option")
    ...
    blockOptions
      .querySelectorAll(".form-select-chosen__option")
      .forEach(element => addEvent(element));
